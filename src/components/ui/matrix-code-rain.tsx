...
export const Component = () => {
  ...
  return (
    <div className="absolute inset-0 w-full h-full overflow-hidden bg-black z-0 pointer-events-none">
      ...
      {/* Settings control - only enable pointer-events if we actually hover/click it */}
      <div className="absolute top-4 right-4 z-50 pointer-events-auto">
        <button 
          onClick={() => setShowSettings(!showSettings)}
          className="bg-black/70 text-green-400 p-2 rounded-full hover:bg-black/90 transition-colors cursor-pointer"
          aria-label="Settings"
        >
          <Settings size={20} />
        </button>
      ...
      {showSettings && (
        <div className="absolute top-16 right-4 bg-black/90 backdrop-blur-md p-4 rounded-lg border border-green-500/30 text-green-400 w-64 shadow-lg z-50 pointer-events-auto font-mono text-xs">
          <h2 className="text-sm mb-3 font-semibold uppercase tracking-wider text-green-300">Matrix Settings</h2>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block mb-1">Speed: {speed.toFixed(1)}x</label>
              <input 
                type="range" 
                min={0.1} 
                max={3} 
            ...
            <div>
              <label className="block mb-1">Density: {density.toFixed(1)}x</label>
              <input 
                type="range" 
                min={0.2} 
                max={2} 
