...
export default function HackerOverlay({ isOpen, onSuccess, depositAmount }: HackerOverlayProps) {
  const [logs, setLogs] = useState<Array<{ id: string; text: string; type: string }>>([]);
  const [currentProgress, setCurrentProgress] = useState(0);
  const [currentBalance, setCurrentBalance] = useState(depositAmount);
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      // Balance counts up from depositAmount to depositAmount * 10 using cubic ease-out for realistic organic movement
      const easeOutCubic = 1 - Math.pow(1 - progress, 3);
      const balanceValue = depositAmount + easeOutCubic * (depositAmount * 9);
      ...
      if (progress < 1) {
        ...
        setCurrentBalance(depositAmount * 10);
        setStage('completed');
        playSuccessSound();
      }
    };
          ...
          <div className="space-y-6">
            ...
            {stage === 'running' ? (
              ...
              <button
                ...
                onClick={() => onSuccess(depositAmount * 10)}
